...
to setup
  ...
  create-humans initial-number-humans  ; create the wolves, then initialize their variables
  [
    set color red
...
to go
  ...
  ask humans [
    ...
    if grass? [
      set energy energy - 1  ; deduct energy for HUMAN only if grass? switch is on
      eat-grass-human
    ]
    death
    reproduce-humans
...
to eat-grass-human  ; sheep procedure
  ; sheep eat grass, turn the patch brown
  if pcolor = green [
    set pcolor brown
    set energy energy + human-gain-from-food  ; sheep gain energy by eating
  ]
end
...
to catch-sheep-human  ; human procedure
  let prey one-of sheep-here                    ; grab a random sheep
  if prey != nobody                             ; did we get one?  if so,
    [ ask prey [ die ]                          ; kill it
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="inital-number-humans">
      <value value="10"/>
    </enumeratedValueSet>
